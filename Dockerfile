...
# The image builds the checked-in source as-is. There is no build-time source
# rewriting: the former 43 patch scripts and .deploy patch set were frozen into
# the modules themselves, so the code reviewed in git is the code that runs here.
# This step only verifies that source.
