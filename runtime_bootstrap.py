...
@media (max-width:768px){
 ...
 /* The reader is assembled and then post-processed in the next frame. Keep its
    first incomplete frame unpainted; visibility preserves layout, so this does
    not replace the flash with a jump or fade. */
...
index=index.replace('/js/app.js?v=20260917_login04','/js/app.js?v=20260917_ptr20',1).replace('/css/styles.css?v=20260917_login04','/css/styles.css?v=20260917_ptr20',1)
mobile_guards=r'''
<script>
(function(){
 ...
 var materialRevealQueued=false;
 function settleMaterial(){
   if(!mobile||materialRevealQueued)return;
   ...
   if(!panel||panel.classList.contains('aula-material-ready'))return;
   materialRevealQueued=true;
   requestAnimationFrame(function(){requestAnimationFrame(function(){panel.classList.add('aula-material-ready');materialRevealQueued=false})});
 }
 ...
 function sync(){lockFields();blurStartup();prep();cleanExtras();cleanDesktop();settleMaterial();requestAnimationFrame(scrollLock)}
