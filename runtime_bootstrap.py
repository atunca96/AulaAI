...
@media (max-width:768px){
 ...
 /* The line above the pedagogy card is reader metadata, not inside .study-card.
    Keep that whole sibling column inside the same reading inset too. */
 .study-meta,.study-meta *,
 .cefr-level-subtitle,.cefr-level-subtitle *,
 #s-ai-book-content-area>.study-meta,#ai-book-content-area>.study-meta{
 ...
 #s-ai-book-content-area .study-card p,#s-ai-book-content-area .study-card li,#s-ai-book-content-area .study-card span,#s-ai-book-content-area .study-card div,#s-ai-book-content-area .study-card code,#s-ai-book-content-area .study-card pre,
 #ai-book-content-area .study-card p,#ai-book-content-area .study-card li,#ai-book-content-area .study-card span,#ai-book-content-area .study-card div,#ai-book-content-area .study-card code,#ai-book-content-area .study-card pre,.vocab-row__ipa,.phonetic-badge{overflow-wrap:anywhere!important;word-break:break-word!important}
 ...
 /* Unit is the parent level; topic must read one step smaller, not equal/larger. */
...
index=index.replace('/js/app.js?v=20260917_login04','/js/app.js?v=20260917_ptr16',1).replace('/css/styles.css?v=20260917_login04','/css/styles.css?v=20260917_ptr16',1)
mobile_guards=r'''
<script>
(function(){
 ...
 function prep(){if(!mobile)return;document.querySelectorAll('.outline__unit').forEach(function(u){var t=u.querySelector(':scope>.outline__unit-title');if(!t||t.dataset.aulaUnitToggle==='1')return;t.dataset.aulaUnitToggle='1';t.setAttribute('role','button');t.setAttribute('tabindex','0');t.setAttribute('aria-expanded','false')})}
 function toggle(t){var u=t.closest('.outline__unit');if(!u)return;var open=!u.classList.contains('is-aula-expanded');document.querySelectorAll('.outline__unit.is-aula-expanded').forEach(function(o){if(o!==u){o.classList.remove('is-aula-expanded');var x=o.querySelector(':scope>.outline__unit-title');if(x)x.setAttribute('aria-expanded','false')}});u.classList.toggle('is-aula-expanded',open);t.setAttribute('aria-expanded',open?'true':'false')}
