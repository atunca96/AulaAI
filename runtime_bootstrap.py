...
@media (max-width: 768px) {
  ...
  /* Generated lesson copy must wrap inside the reader inset. Some generated
     blocks are plain div/span nodes rather than p/li, so constrain descendants
     without changing the reader/card geometry itself. */
  #s-ai-book-content-area .study-card *,
  #ai-book-content-area .study-card * {
    min-width:0 !important;
    max-width:100% !important;
    box-sizing:border-box !important;
  }
  #s-ai-book-content-area .study-card p,
  #s-ai-book-content-area .study-card li,
  #s-ai-book-content-area .study-card span,
  #s-ai-book-content-area .study-card div,
  #s-ai-book-content-area .study-card code,
  #s-ai-book-content-area .study-card pre,
  #ai-book-content-area .study-card p,
  #ai-book-content-area .study-card li,
  #ai-book-content-area .study-card span,
  #ai-book-content-area .study-card div,
  #ai-book-content-area .study-card code,
  #ai-book-content-area .study-card pre,
  .vocab-row__ipa,.phonetic-badge {
  ...
  /* Topics use the exact same compact typographic language as unit headings. */
  .outline__topic {
  ...
  .outline__topic.is-current { color:var(--text-primary); }
...
index = index.replace("/js/app.js?v=20260917_login04", "/js/app.js?v=20260917_ptr15", 1)
index = index.replace("/css/styles.css?v=20260917_login04", "/css/styles.css?v=20260917_ptr15", 1)
...
(function () {
  ...
  function prepareOutline(){ if(!mobile)return; document.querySelectorAll('.outline__unit').forEach(function(unit){ var title=unit.querySelector(':scope > .outline__unit-title'); if(!title||title.dataset.aulaUnitToggle==='1')return; title.dataset.aulaUnitToggle='1'; title.setAttribute('role','button'); title.setAttribute('tabindex','0'); title.setAttribute('aria-expanded','false'); }); }
  function toggleUnit(title){ var unit=title.closest('.outline__unit');if(!unit)return;var opening=!unit.classList.contains('is-aula-expanded');document.querySelectorAll('.outline__unit.is-aula-expanded').forEach(function(other){if(other!==unit){other.classList.remove('is-aula-expanded');var t=other.querySelector(':scope > .outline__unit-title');if(t)t.setAttribute('aria-expanded','false');}});unit.classList.toggle('is-aula-expanded',opening);title.setAttribute('aria-expanded',opening?'true':'false'); }

  function cleanOutlineExtras(){ if(!mobile)return;var panel=document.querySelector('.outline-sheet__panel');if(!panel)return;Array.from(panel.querySelectorAll('button,a,[role="button"],div')).forEach(function(el){if(el.dataset.aulaOutlineCleaned==='1')return;var text=(el.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();var action=((el.getAttribute('onclick')||'')+' '+(el.getAttribute('href')||'')+' '+(el.getAttribute('aria-label')||'')+' '+(el.getAttribute('title')||'')).toLowerCase();var pdf=(/pdf indir|download pdf|pdf download/.test(text)||/pdf/.test(action));var lang=(/arayüz dili\s*:|interface language\s*:/.test(text));if(!pdf&&!lang)return;var row=el;while(row.parentElement&&row.parentElement!==panel&&row.parentElement.children.length===1)row=row.parentElement;if(row!==panel){row.dataset.aulaOutlineCleaned='1';row.remove();}}); }
  function cleanDesktopToolbarPdf(){ if(mobile)return;document.querySelectorAll('.mtoolbar button,.mtoolbar a').forEach(function(el){var hay=((el.textContent||'')+' '+(el.getAttribute('title')||'')+' '+(el.getAttribute('aria-label')||'')+' '+(el.getAttribute('onclick')||'')+' '+(el.getAttribute('href')||'')).toLowerCase();if(/pdf|download/.test(hay))el.remove();}); }
  ...
  function syncOutlineScrollLock(){if(!mobile)return;var open=!!visibleOutlinePanel();if(open&&!outlineLocked){outlineScrollY=window.scrollY||0;document.body.style.position='fixed';document.body.style.top=(-outlineScrollY)+'px';document.body.style.left='0';document.body.style.right='0';document.body.style.width='100%';outlineLocked=true;}else if(!open&&outlineLocked){document.body.style.position='';document.body.style.top='';document.body.style.left='';document.body.style.right='';document.body.style.width='';window.scrollTo(0,outlineScrollY);outlineLocked=false;}}
  ...
  var queued=false;new MutationObserver(function(){if(queued)return;queued=true;requestAnimationFrame(function(){queued=false;sync();});}).observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','style','hidden']});
  document.addEventListener('DOMContentLoaded',function(){sync();requestAnimationFrame(clearStartupFocus);},{once:true});window.addEventListener('pageshow',sync);
  ...
  document.addEventListener('touchstart',unlockTouchedField,{capture:true,passive:true});document.addEventListener('pointerdown',unlockTouchedField,{capture:true,passive:true});
...
print("[BOOT] applied mobile outline typography + lesson text containment")
