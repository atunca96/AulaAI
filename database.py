...
def init_db():
    ...
    with db_connection() as db:
        ...
        # Apply strict source-evidence boundary refresh (v3 - zero lexical pairs/meta-overviews)
        try:
            c.execute("CREATE TABLE IF NOT EXISTS migration_history (key TEXT PRIMARY KEY)")
            mig_done = c.execute("SELECT 1 FROM migration_history WHERE key = 'strict_source_evidence_boundary_refresh_v3'").fetchone()
            if not mig_done:
                json_p = os.path.join(os.path.dirname(__file__), "services", "refreshed_rules_comparisons.json")
                if not os.path.exists(json_p):
                    json_p = os.path.join(os.getcwd(), "services", "refreshed_rules_comparisons.json")
                    ...
                    for tid, raw_c in top_rows:
                        if tid in refresh_map and raw_c:
                            try:
                                ...
                                print(f"[MIGRATION WARNING] Failed to apply v3 boundary refresh to topic {tid}: {ex}")
                    c.execute("INSERT OR IGNORE INTO migration_history (key) VALUES ('strict_source_evidence_boundary_refresh_v3')")
                    print("[MIGRATION] Strict source evidence boundary refresh (v3) applied successfully.")
        except Exception as e:
            print(f"[MIGRATION ERROR] Failed to apply v3 boundary refresh: {e}")
