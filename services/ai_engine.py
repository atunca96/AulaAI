...
def _is_same_batch_semantic_duplicate(item: dict, final_questions: list, topic_content: Any = None) -> bool:
    ...
    # 1. Definitional Concept Grounding Check:
    # Definitional questions for grammatical, rhetorical, semantic, pragmatic, discourse, or literary concepts
    # must be explicitly backed by structured rules or comparisons in metadata.
    def_patterns = [
        r'\b(?:what is|was ist|qué es|que es|qu\'est-ce que|cos\'è|o que é|nedir)\b',
        r'\b(?:definición|definition|tanımı|définition|definizione)\b'
    ]
    is_definitional = any(re.search(pat, p, re.IGNORECASE) for pat in def_patterns)
    if is_definitional and isinstance(topic_content, dict):
        ...
        for pg in topic_content.get("pages", []):
            ...
            for r in pg.get("rules", []):
                if isinstance(r, dict) and r.get("source_evidence"):
                    ...
                    if clean_a in r_name or (len(clean_a) >= 4 and clean_a in r_expl):
            ...
            for c in pg.get("comparisons", []):
                if isinstance(c, dict) and c.get("source_evidence"):
                    ...
                    if clean_a in c_tgt or (len(clean_a) >= 4 and clean_a in c_note):
                        has_backed_rule = True
                        break
            if has_backed_rule:
                break
        # If not explicitly distinguished and supported by structured metadata, reject definitional synthesis
...
"""
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 5 - PLAUSIBLE SAME-LEVEL DISTRACTORS & NATURAL OPTIONS: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically valid, and authentic expression in {language}, with zero malformed or contextually unnatural wording? Every distractor MUST be a real, correctly formed, naturally usable word, phrase, or construction in {language}; NEVER invent, distort, misspell, mechanically alter, or create obviously malformed forms just to make an option incorrect; if natural parallel forms do not exist, use a plausible semantic or functional distractor instead. Stems, answers, and distractors must all be idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options trivially eliminable for the wrong reason. Distractors must be plausible options that compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable, while strictly ensuring no distractor is also valid for the stem. Distractors must be not only grammatical in isolation, but idiomatically plausible in the exact sentence frame of the prompt. A distractor must be wrong because of meaning, pragmatic fit, or context, NEVER because the option itself is ungrammatical, awkward, invented, or malformed! (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, structurally/idiomatically implausible in the sentence frame, or trivially eliminable due to category mismatch). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
