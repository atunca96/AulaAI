...
"""
    ...
    7. DISTRACTOR PLAUSIBILITY, REALISTIC LEARNER CONFUSIONS & CEFR CALIBRATION (CRITICAL):
       ...
       - HIGH-CALIBER DISTRACTOR RIGOR & NEAR-MISS COMPETITIVENESS (NO EASY FILLERS):
         ...
         * COLLOCATIONAL NATURALNESS & ARGUMENT STRUCTURE: Validate collocational naturalness before accepting a question: a word may be semantically related yet must still be rejected if its argument, complement, or surrounding phrase is unnatural in real usage (for example, prefer natural constructions such as “Ekibinizi tenzih ederek...” rather than forcing “tenzih etmek” onto an unnatural abstract object). Do not make questions artificially harder.
    ...
    10. NATURAL & AUTHENTIC LIVING COLLOCATIONS IN {language} (UNIVERSAL FOR ALL TOPICS & LEVELS):
        ...
        - CROSS-LANGUAGE QUALITY, TECHNICAL PRECISION & PEDAGOGICAL APPROPRIATENESS (MANDATORY):
          * Refine cross-language quality: Ensure every item is fully natural and idiomatic in context, uses precise linguistic and domain terminology, has exactly one defensible correct answer, and uses plausible distractors that compete at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable.
          ...
          * Avoid unnecessary repetition of essentially the same target within a single test while still allowing useful coverage of the same broader learning objective.
          * Preserve strict material grounding and CEFR {level} appropriateness, and prefer clarity and authentic usage over artificial difficulty or forced variety.
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 3 - CONTEXTUAL NATURALNESS, TECHNICAL PRECISION & MUTUAL CONSISTENCY: Is every generated item fully natural and idiomatic in context, technically precise in its linguistic and domain terminology, and pedagogically appropriate for the target CEFR {level}? Are stems, answers, and distractors all idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options that are trivially eliminable for the wrong reason? Prefer clarity and authentic usage over artificial difficulty or forced variety. Does the question use linguistically precise terminology when describing grammar, pronunciation, meaning, or usage? Are all verbs, arguments, complements, and surrounding phrases authentic in real living usage (e.g. prefer natural constructions such as “Ekibinizi tenzih ederek...” rather than forcing “tenzih etmek” onto an unnatural abstract object)? Are all lexical choices semantically exact for the context rather than superficially formal or misselected (e.g. reject “salahiyet” where “selamet” is required)? (Resolve all such issues during generation itself; REJECT if artificial, misleading, awkward, linguistically imprecise, or mutually inconsistent).
        * GATE 4 - UNIQUENESS OF THE CORRECT ANSWER (ZERO AMBIGUITY): Does every multiple-choice item have exactly ONE defensible correct answer in the full sentence and context? Before finalizing, ensure no distractor is also grammatically, semantically, pragmatically, or factually valid for the same stem. (Resolve ambiguity during generation itself; REJECT if ambiguous, open to multiple interpretations, or if any distractor could also be defended as valid).
        * GATE 5 - EVERY OPTION MUST BE GRAMMATICALLY NATURAL, IDIOMATICALLY PLAUSIBLE, CATEGORY-MATCHED & COMPETING: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically flawless, and authentic expression in {language}? Ensure stems, answers, and distractors are all idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options that are trivially eliminable for the wrong reason. Are distractors plausible options that compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable, while strictly ensuring no distractor is also valid for the stem? A distractor must be wrong because of meaning, pragmatic fit, or context, NEVER because the option itself is ungrammatical, awkward, invented, or an arbitrary ill-fitting insertion! (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, structurally/idiomatically implausible in the sentence frame, or trivially eliminable due to category mismatch). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
        * GATE 6 - TARGET REPETITION vs. OBJECTIVE COVERAGE: Avoid unnecessary repetition of essentially the same target within a single test while still allowing useful coverage of the same broader learning objective. (REJECT redundant duplication of essentially the same target item, while preserving rich coverage across the material).
    ...
    5) EXACTLY ONE DEFENSIBLE ANSWER & COMPETING DISTRACTORS: Every multiple-choice item MUST have exactly one defensible correct answer in the full sentence and context. Before finalizing, ensure no distractor is also grammatically, semantically, pragmatically, or factually valid for the same stem. Use plausible distractors that compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable. Resolve ambiguity during generation itself. Ensure stems, answers, and distractors are all idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options that are trivially eliminable for the wrong reason. EVERY single option (answer and all 3 distractors) MUST ITSELF be 100% grammatically natural, well-formed, and authentic in {language}. Require distractors to be not only grammatical in isolation, but idiomatically plausible in the exact sentence frame of the prompt. Strictly reject semantically misselected but superficially formal vocabulary in context (e.g. “salahiyet” where “selamet” is required). A distractor must be wrong due to context or meaning, never because the option itself is ungrammatical, awkward, or an invented form. Where appropriate, include at least two plausible near-miss options from the same grammatical or semantic category as the answer. Collocational arguments, complements, and surrounding phrases must be 100% authentic in real usage (e.g. “Ekibinizi tenzih ederek...”). ZERO easy throwaways, filler options, or trivially eliminable distractors.
    6) 100% TARGET LANGUAGE: 'prompt', 'answer', and 'distractors' MUST BE 100% IN {language}.
    7) AVOID UNNECESSARY REPETITION & ALLOW OBJECTIVE COVERAGE: Avoid unnecessary repetition of essentially the same target within a single test while still allowing useful coverage of the same broader learning objective. Every question tests a distinct facet or application; zero redundant duplicate items within the batch.
    ...
    11) CROSS-LANGUAGE QUALITY, TECHNICAL PRECISION & AUTHENTIC USAGE: Refine cross-language quality: ensure every item is fully natural and idiomatic in context, uses precise linguistic and domain terminology, has exactly one defensible correct answer, and uses plausible distractors that compete at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable. Avoid unnecessary repetition of essentially the same target within a single test while still allowing useful coverage of the same broader learning objective. Preserve strict material grounding and CEFR {level} appropriateness, and prefer clarity and authentic usage over artificial difficulty or forced variety. Prompts, scenarios, and all 4 options must flow with effortless native idiomacy, living contemporary vocabulary, and examiner-grade precision. Resolve all of this during generation itself.
    12) PRE-OUTPUT 6-GATE SELF-VERIFICATION: Internally verify each question against the 6 gates (Material grounding, Level fit, Natural context & technical precision, Exactly one defensible correct answer, Plausible category-matched distractors, Target repetition vs objective coverage) before returning JSON. Every multiple-choice item must have exactly one defensible correct answer in the full sentence and context; before finalizing, ensure no distractor is also grammatically, semantically, pragmatically, or factually valid for the same stem. Plausible distractors must compete at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable. Resolve ambiguity and all issues during generation itself. Ensure stems, answers, and distractors are all idiomatic, grammatically valid, functionally plausible, and mutually consistent, with zero artificial wording or misleading terminology."""
