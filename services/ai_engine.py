...
"""
    ...
    10. NATURAL & AUTHENTIC LIVING COLLOCATIONS IN {language} (UNIVERSAL FOR ALL TOPICS & LEVELS):
        ...
        - COMPLETE UTTERANCE INTEGRITY, TECHNICAL PRECISION & PEDAGOGICAL APPROPRIATENESS (MANDATORY):
          * Ensure every generated question is fully natural in context, technically precise in its linguistic terminology, and pedagogically appropriate for the target CEFR {level}.
          ...
          * Use linguistically precise terminology when describing grammar, pronunciation, meaning, or usage (e.g. in questions, stem explanations, grammar labels, and phonetic/semantic descriptions).
          * Resolve all such issues during generation itself so that every delivered item is fully natural, technically precise, and pedagogically appropriate.
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 5 - EVERY OPTION MUST BE GRAMMATICALLY NATURAL, IDIOMATICALLY PLAUSIBLE, CATEGORY-MATCHED & FUNCTIONALLY COMPETING: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically flawless, and authentic expression in {language}? Ensure stems, answers, and distractors are all idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options that are trivially eliminable for the wrong reason. Are distractors not only grammatical in isolation, but idiomatically plausible in the exact sentence frame of the prompt, while strictly ensuring no distractor is also valid for the stem? When possible, do distractors compete with the correct answer at the same linguistic level and functional category (avoiding making the answer obvious by mixing it with options from clearly different grammatical, pragmatic, or register categories)? A distractor must be wrong because of meaning, pragmatic fit, or context, NEVER because the option itself is ungrammatical, awkward, invented, or an arbitrary ill-fitting insertion! (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, or structurally/idiomatically implausible in the sentence frame). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
    ...
    11) NATURAL CONTEXT, TECHNICAL PRECISION & PEDAGOGICAL APPROPRIATENESS: Every generated question must be fully natural in context, technically precise in its linguistic terminology, and pedagogically appropriate for the target CEFR {level}. Ensure stems, answers, and distractors are all idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options that are trivially eliminable for the wrong reason. Prompts, scenarios, and all 4 options must flow with effortless native idiomacy, living contemporary vocabulary, and impeccable grammatical elegance matching official CEFR {level} examinations. Resolve these issues during generation itself.
    12) PRE-OUTPUT 6-GATE SELF-VERIFICATION: Internally verify each question against the 6 gates (Material support, Level fit, Naturalness in context & technical precision, Exactly one defensible correct answer, Distractor plausibility & category matching, Semantic duplication/coverage) before returning JSON. Every multiple-choice item must have exactly one defensible correct answer in the full sentence and context; before finalizing, ensure no distractor is also grammatically, semantically, pragmatically, or factually valid for the same stem. Resolve ambiguity during generation itself. Ensure stems, answers, and distractors are all idiomatic, grammatically valid, functionally plausible, and mutually consistent, with zero artificial wording or misleading terminology."""
