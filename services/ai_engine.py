...
"""
       ...
       - HIGH-CALIBER DISTRACTOR RIGOR & NEAR-MISS COMPETITIVENESS (NO EASY FILLERS):
         ...
         * ZERO easy 'throwaway' or filler options that a student can eliminate at a superficial glance without thinking.
          * EVERY OPTION MUST ITSELF BE GRAMMATICALLY NATURAL & AUTONOMOUSLY WELL-FORMED:
            ...
            - Every distractor must be an authentic, grammatically valid and naturally usable form in the target language. Never invent or deform forms just to create a wrong option; if a natural competing form is unavailable, use another plausible distractor instead.
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 5 - PLAUSIBLE SAME-LEVEL DISTRACTORS & NATURAL OPTIONS: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically valid, and authentic expression in {language}, with zero malformed or contextually unnatural wording? Every distractor must be an authentic, grammatically valid and naturally usable form in the target language. Never invent or deform forms just to create a wrong option; if a natural competing form is unavailable, use another plausible distractor instead. Stems, answers, and distractors must all be idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options trivially eliminable for the wrong reason. Distractors must be plausible options that compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable, while strictly ensuring no distractor is also valid for the stem. Distractors must be not only grammatical in isolation, but idiomatically plausible in the exact sentence frame of the prompt. A distractor must be wrong because of meaning, pragmatic fit, or context, NEVER because the option itself is ungrammatical, awkward, invented, or malformed! (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, structurally/idiomatically implausible in the sentence frame, or trivially eliminable due to category mismatch). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
