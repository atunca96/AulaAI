...
"""
       ...
       - MULTI-PART & OBJECTIVE COVERAGE:
        ...
        - STRICT SOURCE GROUNDING ACROSS ALL LINGUISTIC DOMAINS (CRITICAL MANDATE):
          ...
          * Keep dialogue excluded from target selection: dialogue lines are conversational illustrations, never primary testing targets.
           ...
           * LEXICAL-COMBINATORIAL NATURALNESS & VALENCY IN STEMS AND KEYED ANSWERS (CRITICAL):
             ...
             - Fixed and semi-fixed expressions must preserve their authentic argument structure;
             - When the source does not provide enough evidence to form a natural new combination, DO NOT invent one—use the source-backed expression in a genuinely different context, choose another source-supported target, or generate a natural semantic/functional alternative instead;
     ...
     7. DISTRACTOR PLAUSIBILITY, REALISTIC LEARNER CONFUSIONS & CEFR CALIBRATION (CRITICAL):
        ...
        - HIGH-CALIBER DISTRACTOR RIGOR & NEAR-MISS COMPETITIVENESS (NO EASY FILLERS):
          ...
          * ZERO easy 'throwaway' or filler options that a student can eliminate at a superficial glance without thinking.
           ...
           * LEXICAL HEAD, VALENCY & COMPLEMENT INTEGRITY IN DISTRACTORS (STRICT COMBINATORIAL NATURALNESS):
             - Distractors must strictly preserve the lexical head, valency, complement type, required preposition/case, grammatical compatibility, and collocational relationships demonstrated by authentic living usage and source-backed examples.
             ...
             - Fixed and semi-fixed expressions in distractors must preserve their authentic argument structure and selectional restrictions in full.
             - Inflectional or derivational material must NOT be mechanically mutated or altered to manufacture an incorrect option.
             ...
             - Strict confidence threshold: If the model is not confident that a distractor is 100% natural, idiomatic, and grammatically well-formed in {language}, it MUST avoid that candidate rather than approximate.
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 5 - PLAUSIBLE SAME-LEVEL DISTRACTORS & LEXICAL-COMBINATORIAL INTEGRITY: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically valid, and authentic expression in {language}, with zero malformed or contextually unnatural wording? LEXICAL-COMBINATORIAL NATURALNESS OF DISTRACTORS: Distractors must strictly preserve the lexical head, valency, complement type, required preposition/case, grammatical compatibility, and collocational relationships demonstrated by authentic living usage and source-backed examples. ABSOLUTELY DO NOT mechanically transplant a modifier, prepositional complement, or argument from one taught item onto an incompatible head word or verb merely because both belong to the same semantic domain or lesson page. Distractors must preserve the natural lexical head, valency, complement type, required preposition/case, and authentic grammatical compatibility of each option. Fixed and semi-fixed expressions in distractors must preserve their authentic argument structure and selectional restrictions in full. Inflectional or derivational material must NOT be mechanically mutated or altered to manufacture an incorrect option. When the source does not provide enough evidence to form a natural new combination, DO NOT invent one—use authentic, contextually plausible alternatives from the taught domain instead of fabricating an unnatural near-miss combination. Strict confidence threshold: If the model is not confident that a distractor is 100% natural, idiomatic, and grammatically well-formed in {language}, it MUST avoid that candidate rather than approximate. Every distractor MUST be a real, correctly formed, naturally usable word, phrase, or construction in {language}; NEVER invent, distort, misspell, mechanically alter, or create obviously malformed forms just to make an option incorrect; if natural parallel forms do not exist, use a plausible semantic or functional distractor instead. Stems, answers, and distractors must all be idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options trivially eliminable for the wrong reason. Distractors must be plausible options that compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable, while strictly ensuring no distractor is also valid for the stem. Distractors must be not only grammatical in isolation, but idiomatically plausible in the exact sentence frame of the prompt. A distractor must be wrong because of meaning, pragmatic fit, or context, NEVER because the option itself is ungrammatical, awkward, invented, or malformed! (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, structurally/idiomatically implausible in the sentence frame, or trivially eliminable due to category mismatch). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
        * GATE 6 - IN-BATCH DUPLICATE PREVENTION & DISTINCT OBJECTIVE REUSE: Within the current test and the two retained previous batches, avoid exact or effectively repeated questions. Treat two questions as duplicates when they assess essentially the same target through the same pragmatic situation, communicative purpose, expression, reasoning path, or answer distinction, even if their wording or format differs. Reusing the same learning objective is allowed ONLY when the new question genuinely tests a different application, context, contrast, or cognitive operation; do not include two items that merely paraphrase the same scenario or ask for the same expression/function twice. For grammatical, rhetorical, semantic, pragmatic, discourse, or literary concepts, never synthesize a broad definition from examples or merge neighboring concepts into one another; if the source does not clearly distinguish the target concept, assess recognition or application in context instead. Preserve strict material grounding, CEFR appropriateness, broad unit coverage, and authentic usage; never force novelty at the expense of quality or source fidelity. (REJECT any question that duplicates a target through essentially the same pragmatic situation, communicative purpose, expression, reasoning path, or answer distinction).
    ...
    CRITICAL MANDATES:
    1) STRICT MATERIAL GROUNDING, COMBINATORIAL NATURALNESS & CEFR APPROPRIATENESS: Preserve strict material grounding, CEFR {level} appropriateness, broad unit coverage, and authentic usage; never force novelty at the expense of quality or source fidelity. Questions must assess knowledge, vocabulary, grammar patterns, relationships, or communicative functions explicitly taught or demonstrated in the material. Preserve the lexical and grammatical compatibility demonstrated by source-backed usage: a verb or expression must take a natural subject/object/complement structure; a modifier or complement may not be transplanted onto a different lexical head merely because it is semantically related; fixed and semi-fixed expressions must preserve their authentic argument structure; and inflectional or derivational material must not be mechanically mutated to manufacture an incorrect option. When the source does not provide enough evidence to form a natural new combination, do not invent one—use the source-backed expression in a genuinely different context, choose another source-supported target, or generate a natural semantic/functional alternative instead. Questions may transfer taught knowledge into fresh CEFR-appropriate contexts, but must never require external facts, unstated assumptions, generic world knowledge, or invented lesson content.
...
11) NATURALNESS, TECHNICAL PRECISION, AUTHENTIC USAGE & COMBINATORIAL VALENCY: The model itself must produce fully natural and idiomatic questions, precise linguistic and domain terminology, exactly one defensible correct answer, plausible same-level distractors, and NO malformed or contextually unnatural wording. Stems, keyed answers, and distractors must preserve source-backed lexical valency and combinatorial naturalness; never mutate morphology or transplant complements. Apply strict confidence threshold.
12) PRE-OUTPUT 6-GATE SELF-VERIFICATION (INCLUDING COMBINATORIAL NATURALNESS): Internally verify each question against the 6 gates (Material grounding, Level fit, Natural context & technical precision, Exactly one defensible correct answer, Plausible same-level distractors, Anti-repetition vs. broader objective coverage) before returning JSON. Gate 3 & Gate 5 combinatorial naturalness: strictly verify head-argument, head-modifier, valency, and native construction integrity across stems, keyed answers, and distractors.
