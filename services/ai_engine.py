...
"""
    ...
    10. NATURAL & AUTHENTIC LIVING COLLOCATIONS IN {language} (UNIVERSAL FOR ALL TOPICS & LEVELS):
        ...
        - NATURALNESS, TECHNICAL PRECISION & PEDAGOGICAL APPROPRIATENESS (MANDATORY):
          ...
          * Never invent, distort, or force unnatural word forms just to create parallel distractors; when natural same-form alternatives are unavailable, use plausible alternatives that remain valid language.
          * Keep the answer-option type consistent with what the stem asks for, so a requested term, function, interpretation, or form is answered at the same conceptual level.
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 5 - PLAUSIBLE SAME-LEVEL DISTRACTORS & NATURAL OPTIONS: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically valid, and authentic expression in {language}, with zero malformed or contextually unnatural wording? Keep the answer-option type consistent with what the stem asks for, so a requested term, function, interpretation, or form is answered at the same conceptual level. Never invent, distort, or force unnatural word forms just to create parallel distractors; when natural same-form alternatives are unavailable, use plausible alternatives that remain valid language. Stems, answers, and distractors must all be idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options trivially eliminable for the wrong reason. Distractors must be plausible options that compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable, while strictly ensuring no distractor is also valid for the stem. Distractors must be not only grammatical in isolation, but idiomatically plausible in the exact sentence frame of the prompt. A distractor must be wrong because of meaning, pragmatic fit, or context, NEVER because the option itself is ungrammatical, awkward, invented, or malformed! (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, structurally/idiomatically implausible in the sentence frame, or trivially eliminable due to category mismatch). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
    ...
    12) PRE-OUTPUT 6-GATE SELF-VERIFICATION: Internally verify each question against the 6 gates (Material grounding, Level fit, Natural context & technical precision with no malformed wording, Exactly one defensible correct answer, Plausible same-level distractors, Anti-repetition vs. broader objective coverage) before returning JSON. Every multiple-choice item must have exactly one defensible correct answer in the full sentence and context; ensure no distractor is valid for the same stem. Keep the answer-option type consistent with what the stem asks for, so a requested term, function, interpretation, or form is answered at the same conceptual level. Never invent, distort, or force unnatural word forms just to create parallel distractors; when natural same-form alternatives are unavailable, use plausible alternatives that remain valid language. Distractors must compete at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable. Within the current test and the two retained previous batches, avoid exact or effectively repeated questions (same target in essentially same context/task), while allowing the same broader learning objective to reappear in a genuinely different context or task. Preserve strict material grounding, CEFR appropriateness, broad unit coverage, and authentic usage; never force novelty at the expense of quality or source fidelity. Resolve ambiguity and all issues during generation itself."""
