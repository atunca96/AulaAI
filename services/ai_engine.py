...
def ai_generate_questions(topic_title, topic_type, topic_content, language, count=10, level='A1', existing_questions=None, is_pdf_source=False, is_quiz=False, source_text_override=None, model_override=None, material_language="en", generation_seed=None, focus_directive=None):
    ...
    if source_text_override:
        ...
        parts = [
            ...
            "================================================================================",
            "GROUNDING SAFEGUARDS: Test only source-supported rules and concepts. Distinguish form-inherent",
            "functions from contextual/utterance effects. Keep linguistic concepts definitionally distinct.",
            "--------------------------------------------------------------------------------"
        ]
        ...
        parts = [
            ...
            "GROUNDING SAFEGUARDS: Distinguish strictly between the inherent semantic/grammatical contribution",
            "of a linguistic form and meanings supplied by lexical content, surrounding sentence, discourse",
            "context, or situation. Keep linguistic, grammatical, semantic, and pragmatic categories",
            "definitionally distinct without merging neighboring concepts or non-diagnostic properties.",
...
"""
    ...
    6. STRICT LESSON MATERIAL GROUNDING & LEARNING OBJECTIVES (NOT LITERAL RECALL):
       ...
       - MULTI-PART & OBJECTIVE COVERAGE:
        ...
        - STRICT SOURCE GROUNDING ACROSS ALL LINGUISTIC DOMAINS (CRITICAL MANDATE):
          ...
          * META-LINGUISTIC QUESTIONS & STRUCTURED SOURCE RULES MANDATE: Allow meta-linguistic questions about the meaning or function of a grammatical, semantic, pragmatic, rhetorical, or discourse form ONLY when that exact form–function relationship is explicitly represented in the structured source rules ('[RULE]') or comparisons ('[CONTRAST]'). Do NOT infer such rules from examples, items, dialogue, narrative text, or contextual effects. If no explicit defensible rule exists, test the target through contextual usage, comprehension, lexical meaning, or natural application instead of synthesizing a rule.
          * SAFEGUARD 2 - FORM-INHERENT CONTRIBUTION vs. CONTEXTUAL/UTTERANCE MEANING: Distinguish strictly between the inherent semantic/grammatical contribution of a linguistic form and meanings supplied by the lexical content, surrounding sentence, discourse context, pragmatic situation, register, or real-world interpretation; never attribute contextual effects such as continuity, intensity, irony, skepticism, politeness, certainty, consequence, legal force, evaluation, or interpersonal stance to a form unless the source explicitly teaches that exact form–function relationship. If the intended interpretation belongs to the whole utterance rather than the form itself, ask about the complete utterance instead of claiming that the form encodes it.
          * SAFEGUARD 3 - DEFINITIONALLY DISTINCT CONCEPTS & DIAGNOSTIC PROPERTIES: Keep linguistic, grammatical, semantic, pragmatic, rhetorical, and discourse concepts definitionally distinct: do not merge neighboring categories, context-specific effects, or non-diagnostic properties into a term’s core definition, and only test properties that genuinely distinguish the target concept according to the source.
     ...
     7. DISTRACTOR PLAUSIBILITY, REALISTIC LEARNER CONFUSIONS & CEFR CALIBRATION (CRITICAL):
        ...
        - HIGH-CALIBER DISTRACTOR RIGOR & NEAR-MISS COMPETITIVENESS (NO EASY FILLERS):
          ...
          * ZERO easy 'throwaway' or filler options that a student can eliminate at a superficial glance without thinking.
            * COMPLETE, AUTHENTIC, NATURALLY USABLE LINGUISTIC UNITS (CRITICAL MANDATE):
              - Generate answer choices as complete, authentic, naturally usable linguistic units that fit the stem exactly.
              - NEVER create distractors by mechanically attaching, removing, or mutating morphology, and NEVER use malformed fragments simply to mirror the correct form.
              - A fragment or morphological form may be used as an option ONLY when the source explicitly teaches that exact form and the question genuinely tests form recognition; otherwise use natural full words, phrases, constructions, or contextually plausible alternatives.
              ...
              - SAFEGUARD 1 - AUTHENTIC, CORRECTLY FORMED DISTRACTORS (CRITICAL): Every distractor MUST be an authentic, correctly formed, naturally usable word, phrase, inflected form, or construction in {language}; NEVER invent, deform, misspell, mechanically mutate, or concatenate morphology merely to create an incorrect option. If enough natural parallel distractors do not exist, use plausible semantic, functional, lexical, or contextual alternatives instead.
              - A distractor must be incorrect solely because of the CONTEXT, MEANING, PRAGMATIC FIT, or SUBTLE COLLOCATIONAL MISMATCH with the scenario — NEVER because the option itself is ungrammatical gibberish, an impossible morphological invention, deformed, mechanically mutated, or an unnatural phrase in {language}!
    ...
    10. NATURAL & AUTHENTIC LIVING COLLOCATIONS IN {language} (UNIVERSAL FOR ALL TOPICS & LEVELS):
        ...
        - STRUCTURAL RULE - EXPLICIT SOURCE-SUPPORTED GROUNDING ACROSS ALL DOMAINS (CRITICAL):
          ...
          * Keep dialogue excluded from target selection: dialogue lines are conversational illustrations, never primary testing targets.
        - STRUCTURAL RULE - THREE UNIVERSAL LANGUAGE-AGNOSTIC & CEFR-AGNOSTIC SAFEGUARDS:
          * SAFEGUARD 1 - AUTHENTIC, CORRECTLY FORMED DISTRACTORS & NATIVE CONSTRUCTIONS (CRITICAL):
            - Every distractor MUST be an authentic, correctly formed, naturally usable word, phrase, inflected form, or construction in {language}; NEVER invent, deform, misspell, mechanically mutate, or concatenate morphology merely to create an incorrect option.
            - If enough natural parallel distractors do not exist, use plausible semantic, functional, lexical, or contextual alternatives instead.
            - A distractor must be wrong because of meaning, contextual/pragmatic fit, or subtle collocational mismatch with the scenario — NEVER because the option itself is ungrammatical, deformed, mechanically mutated, or malformed in {language}!
          ...
          * SAFEGUARD 2 - FORM-INHERENT SEMANTIC CONTRIBUTION vs. CONTEXTUAL & UTTERANCE MEANING (CRITICAL):
            - Distinguish strictly between the inherent semantic/grammatical contribution of a linguistic form and meanings supplied by the lexical content, surrounding sentence, discourse context, pragmatic situation, register, or real-world interpretation.
            - NEVER attribute contextual effects such as continuity, intensity, irony, skepticism, politeness, certainty, consequence, legal force, evaluation, or interpersonal stance to a form unless the source explicitly teaches that exact form–function relationship.
            - If the intended interpretation belongs to the whole utterance rather than the form itself, ask about the complete utterance instead of claiming that the form encodes it.
            - For grammar questions, describe only the semantic contribution encoded by the grammatical form itself; never attribute a meaning to the form merely because that meaning is supplied by the lexical verb, surrounding words, discourse context, or real-world situation.
          ...
          * SAFEGUARD 3 - DEFINITIONALLY DISTINCT CONCEPTS & DIAGNOSTIC PROPERTIES (CRITICAL):
            - Keep linguistic, grammatical, semantic, pragmatic, rhetorical, and discourse concepts definitionally distinct.
            - Do NOT merge neighboring categories, context-specific effects, or non-diagnostic properties into a term's core definition.
            - ONLY test properties that genuinely distinguish the target concept according to the source material.
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 3 - CONTEXTUAL NATURALNESS, TECHNICAL PRECISION & ACCURATE EXPLANATIONS: Is every generated question fully natural and idiomatic in context, technically precise in its linguistic and domain terminology, and pedagogically appropriate for the target CEFR {level}? Ensure there is NO malformed or contextually unnatural wording. Allow meta-linguistic questions about the meaning or function of a grammatical, semantic, pragmatic, rhetorical, or discourse form ONLY when that exact form–function relationship is explicitly represented in the structured source rules or comparisons; do NOT infer such rules from examples, items, dialogue, narrative text, or contextual effects. If no explicit defensible rule exists, test the target through contextual usage, comprehension, lexical meaning, or natural application instead of synthesizing a rule. SAFEGUARD 2: Distinguish strictly between the inherent semantic/grammatical contribution of a linguistic form and meanings supplied by the lexical content, surrounding sentence, discourse context, pragmatic situation, register, or real-world interpretation; never attribute contextual effects such as continuity, intensity, irony, skepticism, politeness, certainty, consequence, legal force, evaluation, or interpersonal stance to a form unless the source explicitly teaches that exact form–function relationship. If the intended interpretation belongs to the whole utterance rather than the form itself, ask about the complete utterance instead of claiming that the form encodes it. For grammar questions, describe only the semantic contribution encoded by the grammatical form itself; never attribute a meaning to the form merely because that meaning is supplied by the lexical verb, surrounding words, discourse context, or real-world situation. SAFEGUARD 3: Keep linguistic, grammatical, semantic, pragmatic, rhetorical, and discourse concepts definitionally distinct: do not merge neighboring categories, context-specific effects, or non-diagnostic properties into a term’s core definition, and only test properties that genuinely distinguish the target concept according to the source. When the material teaches an idiom, fixed expression, collocation, discourse marker, or pragmatic phrase, test the whole expression naturally in context instead of decomposing it into morphemes or inventing a grammatical explanation; preserve the expression’s natural argument structure, case marking, collocation, and pragmatic force exactly without synthesizing unnatural variants. For genuine morphology items, test only source-supported grammatical distinctions and use authentic natural alternatives; if a clean morphology question cannot be produced, generate a contextual meaning/usage question instead. Every linguistic, grammatical, pragmatic, or domain explanation must be technically precise and no broader than the source supports; never treat a contextual effect as an inherent meaning of a form. Ensure the stem and keyed answer test exactly the same concept. For conjunction and discourse-marker items, verify that the actual logical relation between clauses exactly matches the target function. Stems, answers, and distractors must all be idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options that are trivially eliminable for the wrong reason. Prefer clarity and authentic usage over artificial difficulty or forced variety. Reject semantically misselected but superficially formal vocabulary in context. Collocational arguments, complements, and surrounding phrases must be authentic in real living usage in {language}. (Resolve all issues during generation itself; REJECT if artificial, malformed, misleading, awkward, linguistically imprecise, or contextually unnatural).
        ...
        * GATE 5 - COMPLETE AUTHENTIC UNITS & PLAUSIBLE SAME-LEVEL DISTRACTORS: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically valid, and authentic expression in {language}, with zero malformed or contextually unnatural wording? Generate answer choices as complete, authentic, naturally usable linguistic units that fit the stem exactly; NEVER create distractors by mechanically attaching, removing, or mutating morphology, and NEVER use malformed fragments simply to mirror the correct form. A fragment or morphological form may be used as an option ONLY when the source explicitly teaches that exact form and the question genuinely tests form recognition; otherwise use natural full words, phrases, constructions, or contextually plausible alternatives. SAFEGUARD 1: Every distractor MUST be an authentic, correctly formed, naturally usable word, phrase, inflected form, or construction in {language}; NEVER invent, deform, misspell, mechanically mutate, or concatenate morphology merely to create an incorrect option. If enough natural parallel distractors do not exist, use plausible semantic, functional, lexical, or contextual alternatives instead. A distractor must be wrong because of meaning, contextual/pragmatic fit, or subtle collocational mismatch with the scenario — NEVER because the option itself is ungrammatical, deformed, mechanically mutated, or malformed in {language}! For fixed expressions and collocations, validate the complete native construction before using it in either the correct answer or a distractor; the expression must take the exact natural person/object/complement, preposition, and case pattern required by real usage in {language}, and an interpretable but non-native collocation is unacceptable. Stems, answers, and distractors must all be idiomatic, grammatically valid, functionally plausible, and mutually consistent, while avoiding artificial wording, misleading terminology, or options trivially eliminable for the wrong reason. Distractors must be plausible options that compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable, while strictly ensuring no distractor is also valid for the stem. Distractors must be not only grammatical in isolation, but idiomatically plausible in the exact sentence frame of the prompt. (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, deformed, mechanically mutated, structurally/idiomatically implausible in the sentence frame, or trivially eliminable due to category mismatch). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
    ...
    CRITICAL MANDATES:
    1) STRICT MATERIAL GROUNDING & CEFR APPROPRIATENESS: Preserve strict material grounding, CEFR {level} appropriateness, broad unit coverage, and authentic usage; never force novelty at the expense of quality or source fidelity. Questions must assess knowledge, vocabulary, grammar patterns, relationships, or communicative functions explicitly taught or demonstrated in the material. Prefer clarity and authentic usage over artificial difficulty or forced variety. Questions may transfer taught knowledge into fresh CEFR-appropriate contexts, but must never require external facts, unstated assumptions, generic world knowledge, or invented lesson content.
    ...
    11) NATURALNESS, TECHNICAL PRECISION & AUTHENTIC USAGE: The model itself must produce fully natural and idiomatic questions, precise linguistic and domain terminology (when describing grammar, pronunciation, meaning, or usage), exactly one defensible correct answer, plausible same-level distractors, and NO malformed or contextually unnatural wording. SAFEGUARD 1: Every distractor must be an authentic, correctly formed, naturally usable word, phrase, inflected form, or construction in {language}; never invent, deform, misspell, mechanically mutate, or concatenate morphology merely to create an incorrect option; if enough natural parallel distractors do not exist, use plausible semantic, functional, lexical, or contextual alternatives instead. For fixed expressions and collocations, validate the complete native construction before using it in either the correct answer or a distractor; the expression must take the exact natural person/object/complement, preposition, and case pattern required by real usage in {language}, and an interpretable but non-native collocation is unacceptable. Preserve authentic argument structure and lexical selection exactly: verify which person, object, case, complement, preposition, or collocate the expression naturally takes in {language}, and never attach the expression to an unnatural object merely because the sentence remains interpretable. SAFEGUARD 2: Distinguish strictly between the inherent semantic/grammatical contribution of a linguistic form and meanings supplied by the lexical content, surrounding sentence, discourse context, pragmatic situation, register, or real-world interpretation; never attribute contextual effects such as continuity, intensity, irony, skepticism, politeness, certainty, consequence, legal force, evaluation, or interpersonal stance to a form unless the source explicitly teaches that exact form–function relationship. If the intended interpretation belongs to the whole utterance rather than the form itself, ask about the complete utterance instead of claiming that the form encodes it. For grammar questions, describe only the semantic contribution encoded by the grammatical form itself; never attribute a meaning to the form merely because that meaning is supplied by the lexical verb, surrounding words, discourse context, or real-world situation. SAFEGUARD 3: Keep linguistic, grammatical, semantic, pragmatic, rhetorical, and discourse concepts definitionally distinct: do not merge neighboring categories, context-specific effects, or non-diagnostic properties into a term’s core definition, and only test properties that genuinely distinguish the target concept according to the source. When the material teaches an idiom, fixed expression, collocation, discourse marker, or pragmatic phrase, test the whole expression naturally in context instead of decomposing it into morphemes or inventing a grammatical explanation. For genuine morphology items, test only source-supported grammatical distinctions and use authentic natural alternatives; if a clean morphology question cannot be produced, generate a contextual meaning/usage question instead. Every linguistic, grammatical, pragmatic, or domain explanation must be technically precise and no broader than the source supports; never treat a contextual effect as an inherent meaning of a form. Ensure the stem and keyed answer test exactly the same concept, and for conjunction/discourse-marker items verify that the actual logical relation between clauses exactly matches the target function. Stems, scenarios, and all 4 options must flow with effortless native idiomacy, living contemporary vocabulary, and examiner-grade precision. Resolve all issues during generation itself.
    12) PRE-OUTPUT 6-GATE SELF-VERIFICATION: Internally verify each question against the 6 gates (Material grounding, Level fit, Natural context & technical precision with accurate explanations and no malformed wording, Exactly one defensible correct answer with verified clause logical relations for connectors, Plausible same-level distractors, Anti-repetition vs. broader objective coverage) before returning JSON. Every multiple-choice item must have exactly one defensible correct answer in the full sentence and context; ensure no distractor is valid for the same stem. Ensure every explanation is technically precise and no broader than the source supports, never treat a contextual effect as an inherent meaning, and ensure the stem and keyed answer test exactly the same concept. Gate 3 & Gate 5 verification: validate complete native constructions for fixed expressions and collocations (must take exact natural person/object/complement, preposition, and case pattern required by real usage; interpretable but non-native collocations are unacceptable); SAFEGUARD 1: verify every distractor is an authentic, correctly formed, naturally usable word, phrase, inflected form, or construction in {language} without invention, deformation, misspelling, mechanical mutation, or morphological concatenation (if natural parallel forms do not exist, use plausible semantic, functional, lexical, or contextual alternatives); SAFEGUARD 2: distinguish strictly between the inherent semantic/grammatical contribution of a linguistic form and meanings supplied by lexical content, surrounding sentence, discourse context, pragmatic situation, register, or real-world interpretation (never attribute contextual effects such as continuity, intensity, irony, skepticism, politeness, certainty, consequence, legal force, evaluation, or interpersonal stance to a form unless explicitly taught; if meaning belongs to the whole sentence, ask about the complete construction or sentence instead of claiming the suffix encodes it); SAFEGUARD 3: keep linguistic, grammatical, semantic, pragmatic, rhetorical, and discourse concepts definitionally distinct without merging neighboring categories, context-specific effects, or non-diagnostic properties into a term's core definition, and only test properties that genuinely distinguish the target concept according to the source. Never attribute pragmatic, rhetorical, continuity, completion, certainty, legal, intensity, or discourse meanings to a suffix, ending, or grammatical construction without explicit source support; test idioms, collocations, and pragmatic phrases as complete natural expressions in context preserving argument structure and force; for genuine morphology items test only source-supported distinctions or generate contextual meaning/usage items instead. Distractors must compete at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable. Within the current test and the two retained previous batches, avoid exact or effectively repeated questions (same target in essentially same context/task), while allowing the same broader learning objective to reappear in a genuinely different context or task. Preserve strict material grounding, CEFR appropriateness, broad unit coverage, and authentic usage; never force novelty at the expense of quality or source fidelity. Resolve ambiguity and all issues during generation itself."""
...
2. DISTINGUISH FORM-INHERENT CONTRIBUTION FROM CONTEXTUAL EFFECTS:
   - Distinguish strictly between the inherent semantic/grammatical contribution of a linguistic form and meanings supplied by the lexical content, surrounding sentence, discourse context, pragmatic situation, register, or real-world interpretation.
   - Do NOT encode contextual effects such as continuity, intensity, irony, skepticism, politeness, certainty, consequence, legal force, evaluation, or interpersonal stance as inherent meanings of a suffix or construction unless the source explicitly teaches that exact form–function relationship.
3. DEFINITIONALLY DISTINCT CONCEPTS & DIAGNOSTIC PROPERTIES:
   - Keep linguistic, grammatical, semantic, pragmatic, rhetorical, and discourse concepts definitionally distinct: do not merge neighboring categories, context-specific effects, or non-diagnostic properties into a term's core definition.
   - Store only properties that genuinely distinguish the target concept according to the source.
...
6. Strict Grammar Rules & Comparisons Grounding (pages[].rules & pages[].comparisons):
   ...
   - Distinguish strictly between the inherent semantic/grammatical contribution of a linguistic form and meanings supplied by the lexical content, surrounding sentence, discourse context, pragmatic situation, register, or real-world interpretation; never attribute contextual effects such as continuity, intensity, irony, skepticism, politeness, certainty, consequence, legal force, evaluation, or interpersonal stance to a form unless the source explicitly teaches that exact form–function relationship.
   - Keep linguistic, grammatical, semantic, pragmatic, rhetorical, and discourse concepts definitionally distinct: do not merge neighboring categories, context-specific effects, or non-diagnostic properties into a term's core definition, and only store properties that genuinely distinguish the target concept according to the source.
   - For discourse markers and constructions, store their core semantic/syntactic function separately from optional pragmatic uses.
