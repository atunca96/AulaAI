...
"""
    ...
    10. NATURAL & AUTHENTIC LIVING COLLOCATIONS IN {language} (UNIVERSAL FOR ALL TOPICS & LEVELS):
        ...
        - NATURALNESS, TECHNICAL PRECISION & PEDAGOGICAL APPROPRIATENESS (MANDATORY):
          ...
          * RULE 1 (FORM vs. CONTEXT ENCODING): Never attribute a meaning to a grammatical form if that meaning comes only from the surrounding sentence or context; describe only what the form itself genuinely encodes. Every linguistic, grammatical, pragmatic, or domain explanation must be technically precise and no broader than the source supports; never treat a contextual effect as an inherent meaning of a form.
          * RULE 2 (NATIVE COLLOCATION & SYNTAX NATURALNESS): Every idiom, fixed expression, collocation, case pattern, and object choice must be fully natural to a native speaker in that exact sentence. Collocational arguments, complements, and surrounding phrases must be authentic in real living usage.
          * RULE 3 (SINGLE DEFENSIBLE ANSWER): Every multiple-choice question must have exactly one defensible correct answer in the complete context, with no distractor that could also reasonably satisfy the stem. Ensure the stem and keyed answer test EXACTLY the same concept. For conjunction and discourse-marker items, verify that the actual logical relation between clauses (e.g. contrast, consequence, addition, concession, cause, condition) exactly matches the target function.
          * RULE 4 (AUTHENTIC & GRAMMATICAL DISTRACTORS): Every distractor must be an authentic, grammatical, naturally usable form or expression—never invent, deform, or mechanically force a form just to make it wrong. If natural parallel distractors do not exist, use another plausible semantic or functional alternative instead. Distractors must compete with the correct answer at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable, while strictly ensuring no distractor is also valid for the stem.
    ...
    12. RIGOROUS LOGICAL FIDELITY & CEFR LEVEL CALIBRATION (UNIVERSAL):
        ...
        - COMPLETE & SELF-CONTAINED CONTEXT & STRICT ANSWER UNIQUENESS:
          * The scenario MUST provide all necessary context so that every multiple-choice question must have exactly one defensible correct answer in the complete context, with no distractor that could also reasonably satisfy the stem.
          * Never attribute a meaning to a grammatical form if that meaning comes only from the surrounding sentence or context; describe only what the form itself genuinely encodes.
          * Every idiom, fixed expression, collocation, case pattern, and object choice must be fully natural to a native speaker in that exact sentence.
          ...
          * Every distractor must be an authentic, grammatical, naturally usable form or expression—never invent, deform, or mechanically force a form just to make it wrong. If natural parallel distractors do not exist, use another plausible semantic or functional alternative instead.
    ...
    11) NATURALNESS, TECHNICAL PRECISION & AUTHENTIC USAGE: The model itself must produce fully natural and idiomatic questions, precise linguistic and domain terminology (when describing grammar, pronunciation, meaning, or usage), exactly one defensible correct answer in the complete context (with no distractor that could also reasonably satisfy the stem), plausible same-level distractors, and NO malformed or contextually unnatural wording. Never attribute a meaning to a grammatical form if that meaning comes only from the surrounding sentence or context; describe only what the form itself genuinely encodes. Every linguistic, grammatical, pragmatic, or domain explanation must be technically precise and no broader than the source supports; never treat a contextual effect as an inherent meaning of a form. Ensure the stem and keyed answer test exactly the same concept, and for conjunction/discourse-marker items verify that the actual logical relation between clauses exactly matches the target function. Every idiom, fixed expression, collocation, case pattern, and object choice must be fully natural to a native speaker in that exact sentence. Every distractor must be an authentic, grammatical, naturally usable form or expression—never invent, deform, or mechanically force a form just to make it wrong; if natural parallel distractors do not exist, use another plausible semantic or functional alternative instead. Stems, scenarios, and all 4 options must flow with effortless native idiomacy, living contemporary vocabulary, and examiner-grade precision. Resolve all issues during generation itself.
    12) PRE-OUTPUT 6-GATE SELF-VERIFICATION: Internally verify each question against the 6 gates (Material grounding, Level fit, Natural context & technical precision with accurate explanations and no malformed wording, Exactly one defensible correct answer with verified clause logical relations for connectors, Plausible same-level distractors, Anti-repetition vs. broader objective coverage) before returning JSON. Never attribute a meaning to a grammatical form if that meaning comes only from the surrounding sentence or context; describe only what the form itself genuinely encodes. Every idiom, fixed expression, collocation, case pattern, and object choice must be fully natural to a native speaker in that exact sentence. Every multiple-choice question must have exactly one defensible correct answer in the complete context, with no distractor that could also reasonably satisfy the stem. Every distractor must be an authentic, grammatical, naturally usable form or expression—never invent, deform, or mechanically force a form just to make it wrong. If natural parallel distractors do not exist, use another plausible semantic or functional alternative instead. Ensure every explanation is technically precise and no broader than the source supports, never treat a contextual effect as an inherent meaning, and ensure the stem and keyed answer test exactly the same concept. Distractors must compete at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable. Within the current test and the two retained previous batches, avoid exact or effectively repeated questions (same target in essentially same context/task), while allowing the same broader learning objective to reappear in a genuinely different context or task. Preserve strict material grounding, CEFR appropriateness, broad unit coverage, and authentic usage; never force novelty at the expense of quality or source fidelity. Resolve ambiguity and all issues during generation itself."""
