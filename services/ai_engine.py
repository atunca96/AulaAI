...
"""
    ...
    10. NATURAL & AUTHENTIC LIVING COLLOCATIONS IN {language} (UNIVERSAL FOR ALL TOPICS & LEVELS):
        ...
        - MORPHOLOGY & FORM-FOCUSED ITEMS (AUTHENTIC FORMS ONLY):
          * For morphology or form-focused multiple-choice items: never create distractors by inventing or mechanically deforming word forms.
          * Use only authentic grammatical forms that genuinely exist in the target language; if enough natural morphological alternatives do not exist, use plausible semantic or functional distractors instead.
        - CONJUNCTIONS & DISCOURSE-MARKER ITEMS (LOGICAL RELATION VALIDATION):
          * For conjunction and discourse-marker items: verify that the logical relation between the clauses genuinely matches the keyed connector, including contrast, confirmation, addition, consequence, concession, or reinforcement.
          * Do not choose a connector merely because it is formally possible; the surrounding propositions must create the exact semantic relation required by that connector.
    ...
    12) PRE-OUTPUT 6-GATE SELF-VERIFICATION: Internally verify each question against the 6 gates (Material grounding, Level fit, Natural context & technical precision with no malformed wording, Exactly one defensible correct answer with verified logical relation for connectors, Plausible same-level distractors with authentic forms only, Anti-repetition vs. broader objective coverage) before returning JSON. Every multiple-choice item must have exactly one defensible correct answer in the full sentence and context; ensure no distractor is valid for the same stem. For conjunction and discourse-marker items, verify that the logical relation between clauses genuinely matches the keyed connector (contrast, confirmation, addition, consequence, concession, or reinforcement; the surrounding propositions must create the exact semantic relation required, never choose merely because formally possible). For morphology or form-focused items, never create distractors by inventing or mechanically deforming word forms; use only authentic grammatical forms that genuinely exist in the target language (or plausible semantic/functional distractors). Distractors must compete at the same grammatical, semantic, pragmatic, or register level rather than being trivially eliminable. Within the current test and the two retained previous batches, avoid exact or effectively repeated questions (same target in essentially same context/task), while allowing the same broader learning objective to reappear in a genuinely different context or task. Preserve strict material grounding, CEFR appropriateness, broad unit coverage, and authentic usage; never force novelty at the expense of quality or source fidelity. Resolve ambiguity and all issues during generation itself."""
