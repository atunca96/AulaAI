...
"""
    ...
    10. NATURAL & AUTHENTIC LIVING COLLOCATIONS IN {language} (UNIVERSAL FOR ALL TOPICS & LEVELS):
        ...
        - COMPLETE UTTERANCE INTEGRITY & LINGUISTIC PRECISION (MANDATORY):
          * Validate each question internally as a complete, natural, context-appropriate utterance in {language}. Prompts, sentence completions, dialogues, and scenario stems must be fully coherent, idiomatic, and pragmatically grounded utterances rather than fragmented or artificial constructions.
          ...
          * Fix such issues internally during generation itself so that every delivered item is a complete, natural utterance with impeccable linguistic precision.
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 3 - COMPLETE NATURAL UTTERANCES, LINGUISTIC PRECISION & SEMANTIC SELECTION: Validate each question internally as a complete, natural, context-appropriate utterance in {language}. Does the question use linguistically precise terminology when describing grammar, pronunciation, meaning, or usage? Are all verbs, arguments, complements, and surrounding phrases authentic in real living usage (e.g. prefer natural constructions such as “Ekibinizi tenzih ederek...” rather than forcing “tenzih etmek” onto an unnatural abstract object)? Are all lexical choices semantically exact for the context rather than superficially formal or misselected (e.g. reject “salahiyet” where “selamet” is required)? (Fix all such issues internally during generation itself; REJECT if incomplete, awkward, linguistically imprecise, or colligationally mismatched).
    ...
    11) COMPLETE NATURAL UTTERANCES & LINGUISTIC PRECISION: Validate each question as a complete, natural, context-appropriate utterance in {language}. Use linguistically precise terminology when describing grammar, pronunciation, meaning, or usage. Prompts, scenarios, and all 4 options must flow with effortless native idiomacy, living contemporary vocabulary, and impeccable grammatical elegance matching official CEFR {level} examinations. Fix any incomplete utterance structures or linguistic imprecisions during generation itself.
    12) PRE-OUTPUT 6-GATE SELF-VERIFICATION: Validate each question as a complete, natural, context-appropriate utterance and verify each against the 6 gates (Material support, Level fit, Naturalness & linguistic precision, Uniqueness of answer, Distractor plausibility & category matching, Semantic duplication/coverage) before returning JSON. Fix any issues during generation itself."""
