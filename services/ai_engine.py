...
def ai_generate_questions(topic_title, topic_type, topic_content, language, count=10, level='A1', existing_questions=None, is_pdf_source=False, is_quiz=False, source_text_override=None, model_override=None, material_language="en", generation_seed=None, focus_directive=None):
    c = int(count)
    gen_count = max(c + 5, int(c * 1.6), 13)
    with open("pipeline.log", "a", encoding="utf-8") as f:
...
"""
    ...
    7. DISTRACTOR PLAUSIBILITY, REALISTIC LEARNER CONFUSIONS & CEFR CALIBRATION (CRITICAL):
       ...
       - HIGH-CALIBER DISTRACTOR RIGOR & NEAR-MISS COMPETITIVENESS (NO EASY FILLERS):
         ...
         * GRAMMATICAL WELL-FORMEDNESS & NATURAL USABILITY: Require EVERY option (answer and all 3 distractors) to be grammatically well-formed and naturally usable in {language}. Avoid distractors that can be eliminated merely because they are malformed, invented, or structurally impossible (e.g. NEVER fabricate artificial suffix combinations like "fark etmeksizinmiş", non-existent verb forms, or broken morphological compounds).
         * At least 1-2 (and where appropriate at least TWO) distractors in EVERY question MUST be plausible near-miss options drawn from the exact same grammatical or semantic category as the answer:
           - In grammar: Use real, well-formed adjacent forms (subtle agreement mismatches, correct tense but wrong grammatical person, subtle word-order inversion errors, or real alternative converbs/suffixes).
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 3 - NATURALNESS & COLLOCATIONAL INTEGRITY: Is the target-language wording 100% natural, idiomatic, and examiner-grade in {language}? Are all verbs, arguments, complements, and surrounding phrases authentic in real living usage (e.g. prefer natural constructions such as “Ekibinizi tenzih ederek...” rather than forcing “tenzih etmek” onto an unnatural abstract object)? (REJECT if awkward, colligationally mismatched, or robotic).
        ...
        * GATE 5 - DISTRACTOR WELL-FORMEDNESS & NEAR-MISS COMPETITIVENESS: Is EVERY option grammatically well-formed and naturally usable in {language}? Avoid distractors that can be eliminated merely because they are malformed, invented, or structurally impossible. Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer? (REJECT if any distractor is an easy throwaway, filler, absurd, or structurally impossible).
    ...
    5) EXACTLY ONE DEFENSIBLE ANSWER & 3 HIGH-CALIBER COMPETITIVE DISTRACTORS: Every question MUST have 1 indisputable correct answer and 3 closely-competing, rigorous distractors. Require EVERY option to be grammatically well-formed and naturally usable in {language}; avoid distractors that can be eliminated merely because they are malformed, invented, or structurally impossible. Where appropriate, include at least two plausible near-miss options from the same grammatical or semantic category as the answer. Collocational arguments, complements, and surrounding phrases must be 100% authentic in real usage (e.g. “Ekibinizi tenzih ederek...”). ZERO easy throwaways or filler options.
