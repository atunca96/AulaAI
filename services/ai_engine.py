...
12) PRE-OUTPUT 6-GATE SELF-VERIFICATION (INCLUDING SYMMETRIC COMBINATORIAL NATURALNESS): Internally verify each question against the 6 gates (Gate 1 Material combination grounding without co-occurrence loophole, Level fit, Natural context & technical precision with scenario-only transfer, Exactly one defensible correct answer with intrinsic keyed-answer combinatorial validation, Plausible same-level distractors with symmetric combinatorial integrity, Anti-repetition vs. broader objective coverage) before returning JSON. Gate 4 must NOT accept a candidate merely because the keyed answer is uniquely distinguishable; the keyed answer itself must first be natural, idiomatic, grammatically well-formed, and combinatorially licensed by source-backed usage. Gate 3 & Gate 5 combinatorial naturalness: strictly verify head-argument, head-modifier, valency, and native construction integrity across stems, keyed answers, and distractors.
