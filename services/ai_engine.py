...
"""
    ...
    13. MANDATORY PRE-OUTPUT INTERNAL VERIFICATION (6 EVALUATION GATES):
        ...
        * GATE 5 - EVERY OPTION MUST ITSELF BE GRAMMATICALLY NATURAL: Does EVERY single option (the correct answer AND all 3 distractors) stand on its own as a completely natural, grammatically flawless, and authentic expression in {language}? A distractor must be wrong because of meaning, pragmatic fit, or context, NEVER because the option itself is ungrammatical, awkward, or invented! (ABSOLUTELY REJECT if any distractor is itself grammatically unnatural, malformed, invented, or structurally impossible). Where appropriate, are at least two distractors plausible near-miss options from the same grammatical or semantic category as the answer?
