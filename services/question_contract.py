...
- ONE VARYING DIMENSION: the four options differ from one another along exactly the dimension the item tests and hold everything else constant. If the item tests person, the options are four persons of one verb in one tense, not one person of four verbs. If it tests which preposition governs a complement, only the preposition moves. An option that differs on two dimensions at once cannot tell you which one the learner got wrong, and is usually eliminable on the dimension you were not asking about.
- ONE MISCONCEPTION EACH: name silently, for each distractor, the single wrong belief that makes a learner choose it. Three distractors are three different beliefs. If two would be chosen for the same reason, one of them is spent: replace it with a different error, or the item is really a three-option item wearing four.
- FRAME FIT: in a lexical, semantic or pragmatic item, all four options are grammatically well-formed where the stem puts them, so the item is decided by meaning, collocation or register and not by agreement. A distractor that does not fit the frame is eliminated without knowing the word, and tests nothing about the word.
...
- REALISTIC ERROR: every distractor is a form a learner at CEFR {level} would actually produce or choose — interference from the language they think in, an overgeneralised rule applied where it does not hold, the right form of the wrong category, a confusable neighbour. A correct option with a diacritic knocked off or a letter dropped is a typo, not a belief: nobody picks it over the correctly spelled word sitting beside it, and it wastes one of only three chances to find out what the learner does not know. So an option set is either ONE word in its competing spellings — a spelling item, where that contrast is the question — or genuinely different forms, each defensible to someone who holds a particular wrong idea. Never a misspelling smuggled in among real forms: "francesa / francés / frances" spends two options on one contrast and one on a slip.
- SHARED TRIGGER: when the stem names a spelling, sound or form feature, every option must carry that feature on the surface, so the item is settled by applying the rule and not by spotting which option contains the letter. "Which word has a silent 'h'" answered by hotel among gato, mesa and casa is not a question — only one option has an h at all. If the feature is exceptionless in {language}, so that every word carrying it would qualify, the item has no single answer as posed: ask what the feature does — how the form is pronounced, what it changes — instead of which word has it.
...
- NEGATIVE AND EXCLUSIVE STEMS: prefer asking which option is right. When the item genuinely has to ask which one is wrong, does not fit or is the exception, say so in words the stem cannot be misread over, and verify that exactly one option fails while all three others genuinely hold — a negative stem with two defensible failures is the commonest way an item ends up with no answer.
...
- ORTHOGRAPHIC CONVENTION: the stem and every option are written the way {language} is written — its own punctuation (including any mark it requires in pairs or at the opening of a clause), its capitalisation rules, its spacing, its diacritics, its script-specific marks. A required accent, capital, particle or opening mark is part of being correct, not decoration, and a learner copies whatever the item prints. Never carry another language's conventions across.
