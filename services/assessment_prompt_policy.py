...
2A. ORDERED-SET LANGUAGE, NOT SEQUENCE TRIVIA
When the lesson contains an ordered lexical set (for example numbers, ordinals, days, months, stages, ranked labels or other sequence-like vocabulary), do not turn the assessment into generic sequence or quantity trivia. Questions such as “what comes next”, “what is between X and Y”, “how many units make X”, or other answers obtainable from ordering/counting alone are low-value unless that ordering relation itself is explicitly the taught linguistic objective. Prefer target-language form, reading/writing, agreement, morphology, collocation, contextual use, pronunciation only when central, and communicative application. This rule is language-agnostic and applies to any ordered lexical system.
...
"""
        ...
        for i, m in enumerate(rewritten):
            ...
            rewritten[i]["content"] = (
                ...
                "For ordered lexical sets, avoid generic next/between/counting or quantity-equivalence trivia unless that relation itself is explicitly the taught linguistic objective; prefer target-language form, usage, agreement, reading/writing and contextual application. "
