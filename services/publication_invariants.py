...
_DOMAIN_GUIDANCE = (
    ...
    "  notation-field-contains-characters-outside-its-notation-system - a transcription contains "
    "characters from another writing system entirely. They are valid Unicode but impossible in "
    "phonetic notation, so the value is corrupt rather than merely inaccurate. Supply the correct "
    "transcription for the whole headword, or answer `omit` - an absent transcription is honest, an "
    "invented one is an error the learner cannot detect.\n"
    "  explanatory-prose-ends-with-a-dangling-notation-fragment - the text ends with a bracketed "
    "fragment that follows the final sentence and belongs to no clause. Either fold it into the "
    "sentence if it was meant to say something, or remove it. Do not leave residue in published prose.\n"
