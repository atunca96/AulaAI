...
- Distractor plausibility & authentic morphological paradigms:
  ...
  * One varying dimension: the 4 options differ along exactly the dimension the item tests and hold everything else constant. If the item tests person, vary the person of one verb in one tense - not four different verbs. An option that differs on two dimensions at once is eliminable on the dimension you were not asking about, and tells you nothing about what the learner actually got wrong.
  * One misconception each: name silently, for each distractor, the single wrong belief that makes a learner choose it. Three distractors are three different beliefs. If two would be chosen for the same reason, replace one - otherwise the item is a three-option item wearing four.
  * Frame fit: in a lexical, semantic or pragmatic item, all 4 options must be grammatically well-formed where the stem puts them, so the item is decided by meaning, collocation or register rather than by agreement. An option that does not fit the frame is eliminated without knowing the word.
  * Negative and exclusive stems: prefer asking which option is correct. If the item must ask which one is wrong or does not fit, make the negation unmissable in the stem and verify that exactly one option fails while all three others genuinely hold.
Orthographic convention: every target-language string in this lesson - example, term, dialogue turn, stem, option - is written the way {language} is written: its own punctuation (including any mark it requires in pairs or at the opening of a clause), capitalisation, spacing, diacritics and script-specific marks. A required accent, capital, particle or opening mark is part of being correct, not decoration, and the learner copies whatever this lesson prints. Never carry another language's conventions across.
</mcq_quality>
